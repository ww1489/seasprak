...
func TestStateRootInsideWorkspaceRejected(t *testing.T) {
	...
	_, err := seasprak.CreateAgentSession(context.Background(), memoryOpts(t, ws, state, "sess-nested"))
	expectCode(t, err, model.CodeInvalidArgument)
}
...
func TestSymlinkDoesNotHideStateOverlap(t *testing.T) {
	...
	_, err := seasprak.CreateAgentSession(context.Background(), memoryOpts(t, link, state, "sess-link"))
	expectCode(t, err, model.CodeInvalidArgument)
}
...
func TestSessionIDAllowsSingleSegmentAndRejectsEscape(t *testing.T) {
	ws := t.TempDir()
	state := t.TempDir()
	s, err := seasprak.CreateAgentSession(context.Background(), memoryOpts(t, ws, state, "sess-reopen"))
	if err != nil {
		t.Fatal(err)
	}
	closeSession(t, s)
	for _, id := range []string{"..", "../outside", "a/b", "a:b", "CON", "com1", "NUL.txt"} {
		_, err := seasprak.CreateAgentSession(context.Background(), memoryOpts(t, ws, state, id))
		expectCode(t, err, model.CodeInvalidArgument)
	}
}
...
func TestCreateConflictsWithoutOverwritingJournal(t *testing.T) {
	...
	_, err = seasprak.CreateAgentSession(context.Background(), opts)
	expectCode(t, err, model.CodeStateConflict)
...
func TestStartErrorReleasesStore(t *testing.T) {
	...
	opts.Profile = session.ProfileDefault
	_, err := seasprak.CreateAgentSession(context.Background(), opts)
	if err == nil {
		t.Fatal("default profile must fail")
	}
	opts.Profile = session.ProfileMemory
	s, err := seasprak.OpenAgentSession(context.Background(), opts)
	if err != nil {
		t.Fatal(err)
	}
	closeSession(t, s)
...
func TestStoreOpenErrorDropsManifest(t *testing.T) {
	...
	defer lock.Unlock()
	_, err = seasprak.CreateAgentSession(context.Background(), memoryOpts(t, ws, state, "sess-locked"))
	if err == nil {
...
func TestOpenDoesNotCreateMissingSession(t *testing.T) {
	state := t.TempDir()
	_, err := seasprak.OpenAgentSession(context.Background(), memoryOpts(t, t.TempDir(), state, "sess-missing"))
	expectCode(t, err, model.CodeNotFound)
...
func TestOpenRestoresWorkspaceAndRejectsSwitch(t *testing.T) {
	...
	_, err = seasprak.OpenAgentSession(context.Background(), opts)
	expectCode(t, err, model.CodeStateConflict)
	opts.Workspace = ""
...
func TestMatchingToolInfoAccepted(t *testing.T) {
	...
	opts.ToolInfos = []*schema.ToolInfo{seasprak.ToolInfo("add", "add a number")}
	s, err := seasprak.CreateAgentSession(context.Background(), opts)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenRejectsSwappedToolDeclaration(t *testing.T) {
	...
	defer cancel()
	if _, err := s.SubmitInput(ctx, agent.InputCommand{Kind: "prompt", Content: json.RawMessage(`{"text":"hello"}`)}); err != nil {
		t.Fatal(err)
	}
	closeSession(t, s)
	...
	_, err = seasprak.OpenAgentSession(context.Background(), opts)
	expectCode(t, err, model.CodeIncompatibleVersion)
...
func TestNoToolsUseBuiltinGeneration(t *testing.T) {
	ws := t.TempDir()
	state := t.TempDir()
	s, err := seasprak.CreateAgentSession(context.Background(), memoryOpts(t, ws, state, "sess-builtin"))
	if err != nil {
		t.Fatal(err)
	}
	closeSession(t, s)
...
func memoryOpts(t *testing.T, workspace, state, id string) session.Options {
	t.Helper()
	return session.Options{
		Workspace: workspace, StateRoot: state, SessionID: id,
		Profile: session.ProfileMemory, Model: testkit.NewFake(testkit.Step{Text: "done"}),
