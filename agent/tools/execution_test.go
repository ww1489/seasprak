...
func TestInvalidArgumentsDoNotRun(t *testing.T) {
	...
	}}, sink, allow{}, agent.NewBudget(model.DefaultLimits()))
	if err != nil {
		t.Fatal(err)
	}
...
func (s *recordSink) LookupTool(context.Context, agent.ExecutionScope, string) (agent.ToolRecord, error) {
	if !s.found {
		return agent.ToolRecord{}, model.NewError(model.CodeNotFound, "not accepted")
	}
	return s.rec, nil
}
...
func TestAcceptedSchemaDenyAndBudgetPersistObservation(t *testing.T) {
	...
	if !ok || pe.Code != model.CodeBudgetExhausted {
		t.Fatalf("err=%v", err)
	}
	...
	exec, err = NewExecutor("gen", []Definition{addDef(func(context.Context, json.RawMessage) (string, error) {
		calls++
		return "1", nil
	})}, sink, deny{}, agent.NewBudget(model.DefaultLimits()))
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntentIsPersistedBeforeBudget(t *testing.T) {
	budg := agent.NewBudget(model.DefaultLimits())
	sink := &recordSink{found: true, budg: budg, rec: accepted(`{"n":1}`)}
	exec, err := NewExecutor("gen", []Definition{addDef(func(context.Context, json.RawMessage) (string, error) {
		return "1", nil
	})}, sink, allow{}, budg)
...
func TestNilRunAndPanicDoNotRerun(t *testing.T) {
	sink := &recordSink{found: true, rec: accepted(`{"n":1}`)}
	exec, err := NewExecutor("gen", []Definition{addDef(nil)}, sink, allow{}, agent.NewBudget(model.DefaultLimits()))
	if err != nil {
		t.Fatal(err)
	}
	out, err := exec.Run(context.Background(), agent.ExecutionScope{}, "prov-1", "add", `{"n":1}`)
