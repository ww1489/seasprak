...
func TestBeginTurnCountsLogicalAndOccupyCountsTransport(t *testing.T) {
	limits := model.Limits{LogicalModelRequests: 2, TraceLogicalModelCalls: 2, TraceTransportRequests: 5, TraceToolCalls: 3}
	b := NewBudget(limits)
	var persisted []Usage
...
func TestPersistFailureDoesNotSwapUsage(t *testing.T) {
	b := NewBudget(model.Limits{TraceLogicalModelCalls: 4, TraceTransportRequests: 4, TraceToolCalls: 2})
...
func TestRestoreReplacesUsage(t *testing.T) {
	b := NewBudget(model.DefaultLimits())
