...
func TestNoToolFinishTurnCarriesID(t *testing.T) {
	...
	ag, err := NewAgent(context.Background(), Deps{
		Model: fake, Sink: nopFact{}, Budget: agent.NewBudget(product.DefaultLimits()),
...
func TestRetriesOnlyRetryableErrorsWithinBudget(t *testing.T) {
	...
	budg := agent.NewBudget(product.DefaultLimits())
	ag, err := NewAgent(context.Background(), Deps{
	...
	ag, err = NewAgent(context.Background(), Deps{
		Model: permanent, Sink: nopFact{}, Budget: agent.NewBudget(product.DefaultLimits()), Instruction: "test",
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPipelineUsesProviderCallIDAndTurnScope(t *testing.T) {
	...
	}}, sink, allowAll{}, agent.NewBudget(product.DefaultLimits()))
	if err != nil {
		t.Fatal(err)
	}
	...
	ag, err := NewAgent(context.Background(), Deps{
		...
		Sink: sink, Budget: agent.NewBudget(product.DefaultLimits()), Boundary: boundary,
