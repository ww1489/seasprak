...
func TestStreamCancellationBeforeAcceptance(t *testing.T) {
	...
	vm := NewValidatedModel(malformedModel{msg: msg, cancel: cancel}, &factSink{}, agent.NewBudget(model.DefaultLimits()), agent.ExecutionScope{})
...
func TestMalformedContentBlocksFailClosed(t *testing.T) {
	for _, blocks := range [][]*schema.ContentBlock{{nil}, {{Type: schema.ContentBlockTypeFunctionToolCall}}} {
		t.Run("malformed", func(t *testing.T) {
			...
			vm := NewValidatedModel(malformedModel{msg: msg}, &factSink{}, agent.NewBudget(model.DefaultLimits()), agent.ExecutionScope{})
