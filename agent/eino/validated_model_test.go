...
func TestStreamEOFWithoutFinishDoesNotAcceptTools(t *testing.T) {
	...
	sink := &factSink{}
	vm := NewValidatedModel(fake, sink, agent.NewBudget(model.DefaultLimits()), agent.ExecutionScope{})
...
func TestAcceptsToolCallsWithExplicitFinish(t *testing.T) {
	for _, finish := range []string{"stop", "tool_calls"} {
		t.Run(finish, func(t *testing.T) {
			...
			sink := &factSink{}
			vm := NewValidatedModel(fake, sink, agent.NewBudget(model.DefaultLimits()), agent.ExecutionScope{TraceID: "tr"})
...
func TestFailureRecordPersistErrorIsReturned(t *testing.T) {
	fake := testkit.NewFake(testkit.Step{Err: model.NewError(model.CodeResourceUnavailable, "down")})
