...
func (m AgentMessage) Validate() error {
	if m.ID == "" {
		return product.NewError(product.CodeInvalidArgument, "message id is required")
	}
	var n int
	if m.Standard != nil {
		n++
	...
	if n != 1 {
		return product.NewError(product.CodeInvalidArgument, "message must contain exactly one payload")
	}
	switch m.Kind {
	case KindUser, KindAssistant, KindToolResult:
		if m.Standard == nil {
			return product.Errorf(product.CodeInvalidArgument, "kind %s requires standard payload", m.Kind)
		}
	case KindCustom:
		if m.Custom == nil {
			return product.NewError(product.CodeInvalidArgument, "custom kind requires custom payload")
		}
	case KindCommand:
		if m.Command == nil {
			return product.NewError(product.CodeInvalidArgument, "command kind requires command payload")
		}
	case KindCompactionSummary, KindBranchSummary:
		if m.Summary == nil {
			return product.NewError(product.CodeInvalidArgument, "summary kind requires summary payload")
		}
	case KindOpaque:
		if m.Opaque == nil {
			return product.NewError(product.CodeInvalidArgument, "opaque kind requires opaque payload")
		}
	default:
		return product.Errorf(product.CodeInvalidArgument, "unknown message kind %s", m.Kind)
	}
	return nil
}
