...
func TestMissingWorkspace(t *testing.T) {
	_, err := sessions.CreateAgentSession(context.Background(), sessions.Options{Profile: sessions.ProfileMemory})
	if err == nil {
...
func TestReopenDoesNotRun(t *testing.T) {
	...
	opts := sessions.Options{
		Workspace: ws, StateRoot: root, SessionID: "sess-reopen", Profile: sessions.ProfileMemory, Model: fake,
	...
	s, err := sessions.CreateAgentSession(context.Background(), opts)
	if err != nil {
		t.Fatal(err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	...
	reopened, err := sessions.OpenAgentSession(context.Background(), opts)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDefaultProfileRejectsMissingBackends(t *testing.T) {
	_, err := sessions.CreateAgentSession(context.Background(), sessions.Options{Workspace: t.TempDir(), Profile: sessions.ProfileDefault})
	if err == nil {
		t.Fatal("default profile must fail closed")
	}
}

func openSession(t *testing.T, fake *testkit.FakeModel, run func(context.Context, json.RawMessage) (string, error)) *sessions.AgentSession {
	t.Helper()
	if run == nil {
		run = func(context.Context, json.RawMessage) (string, error) { return "ok", nil }
	}
	_ = run
	s, err := sessions.CreateAgentSession(context.Background(), sessions.Options{
		Workspace: t.TempDir(), StateRoot: t.TempDir(), Profile: sessions.ProfileMemory, Model: fake,
		Instruction: "memory tools only",
		Tools: []tools.Definition{{
...
func closeSession(t *testing.T, s *sessions.AgentSession) {
	t.Helper()
...
func waitState(t *testing.T, s *sessions.AgentSession, traceID, want string) {
	t.Helper()
