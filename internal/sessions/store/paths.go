...
func ResolveDir(path string) (string, error) {
	if strings.TrimSpace(path) == "" {
		return "", product.NewError(product.CodeInvalidArgument, "directory path is required")
	}
	...
	if err != nil {
		return "", product.NewError(product.CodeInvalidArgument, "directory is unavailable")
	}
	...
	if !info.IsDir() {
		return "", product.NewError(product.CodeInvalidArgument, "path is not a directory")
	}
...
func ValidateResourceID(id string) error {
	if id == "" || len(id) > 128 || id != strings.TrimSpace(id) {
		return product.NewError(product.CodeInvalidArgument, "resource id must be one safe path segment")
	}
	...
	case "CON", "PRN", "AUX", "NUL":
		return product.NewError(product.CodeInvalidArgument, "resource id uses a reserved device name")
	}
