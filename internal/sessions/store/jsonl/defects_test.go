...
func TestSyncFailureKeepsUncertainBytes(t *testing.T) {
	root := t.TempDir()
	calls := 0
	store, err := jsonl.Open("s1", root, contract.Header{}, jsonl.Options{SyncFile: func(f *os.File) error {
		calls++
		if calls > 1 {
			return os.ErrClosed
		}
	...
	_, err = store.Append(context.Background(), "s1", contract.ExpectedCommit{IdempotencyKey: "k", ContentDigest: []byte("d")}, contract.Commit{CommitID: "c1"})
	...
	if _, err := store.Append(context.Background(), "s1", contract.ExpectedCommit{ExpectedPreviousSeq: 1}, contract.Commit{CommitID: "c2"}); err == nil {
		t.Fatal("writes must stop after an uncertain commit")
	}
	if err := store.Close(); err != nil {
		t.Fatal(err)
	}
	again, err := jsonl.Open("s1", root, contract.Header{}, jsonl.Options{})
	if err != nil {
		t.Fatal(err)
	}
	defer again.Close()
	dup, err := again.Append(context.Background(), "s1", contract.ExpectedCommit{IdempotencyKey: "k", ContentDigest: []byte("d")}, contract.Commit{CommitID: "other"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestShortWriteDoesNotTruncate(t *testing.T) {
	root := t.TempDir()
	writes := 0
	store, err := jsonl.Open("s1", root, contract.Header{}, jsonl.Options{Write: func(f *os.File, p []byte) (int, error) {
		writes++
		if writes == 1 {
			return f.Write(p)
		}
	...
	_, err = store.Append(context.Background(), "s1", contract.ExpectedCommit{}, contract.Commit{CommitID: "c1"})
	pe, ok := err.(*product.Error)
	if !ok || pe.Code != product.CodeStorageUnavailable {
		t.Fatalf("err = %v", err)
	}
...
func TestLineLargerThanScannerDefault(t *testing.T) {
	root := t.TempDir()
	store, err := jsonl.Open("s1", root, contract.Header{}, jsonl.Options{})
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
	payload := append(append([]byte(`{"text":"`), bytes.Repeat([]byte("a"), 70*1024)...), '"', '}')
	_, err = store.Append(context.Background(), "s1", contract.ExpectedCommit{}, contract.Commit{
		CommitID: "c1",
		Entries:  []contract.Record{{Type: "message", Version: 1, ID: "m1", Payload: payload}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestValidJSONWithoutNewlineIsNotCommitted(t *testing.T) {
	...
	if _, err := store.Append(context.Background(), "s1", contract.ExpectedCommit{}, contract.Commit{CommitID: "c1"}); err != nil {
		t.Fatal(err)
	}
	store.Close()
	path := journalPath(root)
	...
	body, _ := json.Marshal(contract.Commit{RecordType: "commit", Version: 1, CommitID: "c2", CommitSeq: 2, ExpectedPreviousSeq: 1})
	if _, err := f.Write(body); err != nil {
		t.Fatal(err)
	}
	f.Close()
	reopen, err := jsonl.Open("s1", root, contract.Header{}, jsonl.Options{})
	if err != nil {
		t.Fatal(err)
	}
	defer reopen.Close()
	...
	if _, err := reopen.Append(context.Background(), "s1", contract.ExpectedCommit{ExpectedPreviousSeq: 1}, contract.Commit{CommitID: "c3"}); err == nil {
...
func TestMiddleDamageIsRejected(t *testing.T) {
	...
	_, err = jsonl.Open("s1", root, contract.Header{}, jsonl.Options{})
	if err == nil {
...
func TestFutureVersionAndChainValidation(t *testing.T) {
	...
	root = t.TempDir()
	seq := uint64(5)
	body, _ := json.Marshal(contract.Commit{
...
func TestReadAfterAndCopy(t *testing.T) {
	...
	if _, err := store.Append(context.Background(), "s1", contract.ExpectedCommit{}, contract.Commit{
		CommitID: "c1",
		Entries:  []contract.Record{{Type: "message", Version: 1, ID: "m1", Payload: []byte(`{"n":1}`)}},
	}); err != nil {
		t.Fatal(err)
	}
...
func TestCrossProcessWriterExcluded(t *testing.T) {
	if os.Getenv("SEASPRAK_LOCK_CHILD") == "1" {
		_, err := jsonl.Open("s1", os.Getenv("SEASPRAK_LOCK_ROOT"), contract.Header{}, jsonl.Options{})
		if err != nil {
			os.Exit(2)
		}
		os.Exit(0)
...
func TestCommitCrashKeepsPrefix(t *testing.T) {
	...
	_, err = reopen.Append(context.Background(), "s1", contract.ExpectedCommit{ExpectedPreviousSeq: 1}, contract.Commit{CommitID: "c2"})
	if err == nil {
...
func TestOpenExistingDoesNotCreate(t *testing.T) {
	root := t.TempDir()
	_, err := jsonl.Open("missing", root, contract.Header{}, jsonl.Options{OpenExisting: true})
	pe, ok := err.(*product.Error)
	...
	created.Close()
	again, err := jsonl.Open("s1", root, contract.Header{}, jsonl.Options{OpenExisting: true})
	if err != nil {
		t.Fatal(err)
	}
	again.Close()
...
func TestSessionIDAndSymlinkEscape(t *testing.T) {
	...
	for _, id := range []string{"..", "../x", `a\b`, "a/b", `C:s`, "/abs"} {
		if _, err := jsonl.Open(id, root, contract.Header{}, jsonl.Options{}); err == nil {
...
func headerLine() string {
	raw, _ := json.Marshal(contract.Header{RecordType: "header", FormatVersion: 1, SessionID: "s1"})
	return string(raw)
}

func commitLine(id string, seq, prev uint64) string {
	raw, _ := json.Marshal(contract.Commit{
...
func commitWithEntry(id string, seq, prev uint64, entryID, parent string) string {
	raw, _ := json.Marshal(contract.Commit{
		RecordType: "commit", Version: 1, CommitID: id, CommitSeq: seq, ExpectedPreviousSeq: prev,
		Entries: []contract.Record{{Type: "message", Version: 1, ID: entryID, ParentID: parent, Payload: []byte(`{}`)}},
	})
	return string(raw)
}
...
func commitWithControlParent(id string, seq, prev uint64, parent string) string {
	raw, _ := json.Marshal(contract.Commit{
		RecordType: "commit", Version: 1, CommitID: id, CommitSeq: seq, ExpectedPreviousSeq: prev,
		ControlRecords: []contract.Record{{Type: "budget", Version: 1, ID: "s1", ParentID: parent, Payload: []byte(`{}`)}},
	})
	return string(raw)
}
