...
func (s *failingStore) Append(ctx context.Context, id string, e store.ExpectedCommit, c store.Commit) (store.CommitReceipt, error) {
	if s.fail {
		return store.CommitReceipt{}, errors.New("injected sync failure")
	}
	return s.Store.Append(ctx, id, e, c)
}
func fixture(t *testing.T) (*state.Manager, *failingStore) {
	t.Helper()
	s, err := memory.Open("session", store.Header{})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { s.Close() })
	f := &failingStore{Store: s}
	m, err := state.NewManager(f, "session")
	if err != nil {
		t.Fatal(err)
	}
	return m, f
}
func accept(t *testing.T, m *state.Manager, key string, content string) agent.InputReceipt {
	t.Helper()
