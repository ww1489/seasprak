...
func TestCursorSurvivesCommitWithoutEvents(t *testing.T) {
	store := openMem(t)
	if _, err := store.Append(context.Background(), "s1", history.ExpectedCommit{}, commitWithEvent("c1", "e1", 0)); err != nil {
		t.Fatal(err)
	}
	...
	receipt, err := store.Append(context.Background(), "s1", history.ExpectedCommit{ExpectedPreviousSeq: 2}, commitWithEvent("c3", "e2", 2))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadCopiesDoNotAliasStore(t *testing.T) {
	...
	if _, err := store.Append(context.Background(), "s1", history.ExpectedCommit{}, history.Commit{
		CommitID: "c1",
		Entries:  []history.Record{{Type: "message", Version: 1, ID: "m1", Payload: []byte(`{"text":"a"}`)}},
	}); err != nil {
		t.Fatal(err)
	}
	loaded, err := store.Load(context.Background(), "s1")
...
func TestSameKeyDifferentDigestConflicts(t *testing.T) {
	store := openMem(t)
	key := history.ExpectedCommit{IdempotencyKey: "k", ContentDigest: []byte("one")}
	if _, err := store.Append(context.Background(), "s1", key, history.Commit{CommitID: "c1"}); err != nil {
		t.Fatal(err)
	}
	_, err := store.Append(context.Background(), "s1", history.ExpectedCommit{IdempotencyKey: "k", ContentDigest: []byte("two")}, history.Commit{CommitID: "c2"})
	pe, ok := err.(*model.Error)
	if !ok || pe.Code != model.CodeIdempotencyConflict {
...
func TestRejectsSessionIDOutsideOneSegment(t *testing.T) {
	for _, id := range []string{"../x", `a\b`, "a/b", `C:s`, `/abs`, `\\srv\s`} {
		_, err := memory.Open(id, history.Header{})
		if err == nil {
...
func openMem(t *testing.T) *memory.Store {
	t.Helper()
	store, err := memory.Open("s1", history.Header{})
	if err != nil {
		t.Fatal(err)
	}
...
func appendSeq(t *testing.T, store *memory.Store, id string, prev uint64) {
	t.Helper()
	_, err := store.Append(context.Background(), "s1", history.ExpectedCommit{ExpectedPreviousSeq: prev}, history.Commit{CommitID: id, ExpectedPreviousSeq: prev})
	if err != nil {
		t.Fatal(err)
	}
}

func commitWithEvent(commitID, eventID string, prev uint64) history.Commit {
