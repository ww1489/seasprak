...
func TestSubscriptionCountsPendingNotLifetime(t *testing.T) {
	sub := &subscription{ch: make(chan agent.Event), wake: make(chan struct{}, 1), done: make(chan struct{}), limits: model.Limits{SubscriptionEvents: 2, SubscriptionBytes: 4096}}
	go sub.deliver()
	defer sub.close(nil)
	for i := 0; i < 100; i++ {
...
func TestSubscriptionOverflowReportsResync(t *testing.T) {
	sub := &subscription{ch: make(chan agent.Event), wake: make(chan struct{}, 1), done: make(chan struct{}), limits: model.Limits{SubscriptionEvents: 1, SubscriptionBytes: 4096}}
	...
	pe, ok := model.AsError(public.Err())
	if !ok || pe.Code != model.CodeResyncRequired {
