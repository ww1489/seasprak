...
func (rt *runtime) executeSegment(frame *execution, inputID string) error {
	...
	loop := adk.NewTurnLoop(adk.TurnLoopConfig[agent.InputRef, *schema.AgenticMessage]{
		GenInput: func(ctx context.Context, _ *adk.TurnLoop[agent.InputRef, *schema.AgenticMessage], items []agent.InputRef) (*adk.GenInputResult[agent.InputRef, *schema.AgenticMessage], error) {
			if len(items) == 0 {
				return nil, model.NewError(model.CodeInternal, "execution input is missing")
			}
			value, err := rt.call(ctx, func(rt *runtime) (any, error) {
				if rt.active != frame {
					return nil, model.NewError(model.CodeStateConflict, "execution is no longer active")
				}
	...
	if pushed, _ := loop.Push(agent.InputRef{InputID: inputID, TraceID: scope.TraceID, Kind: "prompt"}); !pushed {
		return model.NewError(model.CodeStateConflict, "execution loop rejected input")
	}
	loop.Run(ctx)
	exit := loop.Wait()
	...
	if len(exit.UnhandledItems) > 0 || len(exit.InterruptedItems) > 0 || len(exit.TakeLateItems()) > 0 {
		return model.NewError(model.CodeStateConflict, "execution ended with unhandled input")
	}
	return ctx.Err()
}
func (rt *runtime) CommitFact(ctx context.Context, scope agent.ExecutionScope, fact agent.Fact) error {
	...
	return rt.do(context.WithoutCancel(ctx), func(rt *runtime) error {
		if rt.active == nil || rt.active.scope.TraceID != scope.TraceID || rt.active.scope.InvocationID != scope.InvocationID {
			return model.NewError(model.CodeStateConflict, "execution scope is not active")
		}
		if scope.TurnID == "" {
		...
		case "assistant":
			...
			if body.Message == nil {
				return model.NewError(model.CodeInvalidArgument, "assistant response is empty")
			}
			msg := assistantMessage(scope, body.Message)
		...
		case "tool_intent":
			...
			if !ok || call.Scope.TurnID != scope.TurnID || call.Call != frozen {
				return model.NewError(model.CodeStateConflict, "tool intent does not match accepted call")
			}
			if call.Claimed {
				return model.NewError(model.CodeReconciliationRequired, "tool execution was already claimed")
			}
			call.Claimed = true
		...
		case "tool_observation":
			...
			if !ok || old.Call != call.Call || old.Scope != call.Scope || call.Observation == nil {
				return model.NewError(model.CodeStateConflict, "tool observation does not match accepted call")
			}
...
func (rt *runtime) LookupTool(ctx context.Context, scope agent.ExecutionScope, providerID string) (agent.ToolRecord, error) {
	value, err := rt.call(ctx, func(rt *runtime) (any, error) {
		if rt.active == nil || rt.active.scope.TraceID != scope.TraceID || rt.active.scope.InvocationID != scope.InvocationID {
			return nil, model.NewError(model.CodeStateConflict, "tool execution is not active")
		}
		if scope.TurnID == "" {
			scope.TurnID = rt.active.turnID
		}
		...
		return nil, model.NewError(model.CodeStateConflict, "tool call was not accepted")
	})
	if err != nil {
...
func (rt *runtime) PrepareNextTurn(ctx context.Context, scope agent.ExecutionScope) (agent.TurnPlan, error) {
	value, err := rt.call(ctx, func(rt *runtime) (any, error) {
		if rt.active == nil || rt.active.scope.TraceID != scope.TraceID {
			return nil, model.NewError(model.CodeStateConflict, "trace is not active")
		}
		if err := rt.active.ctx.Err(); err != nil {
			return nil, err
		}
		if rt.active.turnID != "" {
			...
			if !previous.Ended {
				return nil, model.NewError(model.CodeStateConflict, "previous turn is unfinished")
			}
		}
...
func (rt *runtime) FinishTurn(ctx context.Context, scope agent.ExecutionScope, fact agent.TurnFact) error {
	return rt.do(context.WithoutCancel(ctx), func(rt *runtime) error {
		if rt.active == nil || rt.active.scope.TraceID != scope.TraceID {
			return model.NewError(model.CodeStateConflict, "trace is not active")
		}
		id := fact.TurnID
		if id == "" {
			id = scope.TurnID
		...
		if !ok || turn.TraceID != scope.TraceID {
			return model.NewError(model.CodeStateConflict, "turn is not active")
		}
