...
func TestRuntimeRetryUsesSameLogicalTurn(t *testing.T) {
	fake := testkit.NewFake(testkit.Step{Err: &model.Error{Code: model.CodeResourceUnavailable, Message: "temporary", Retryable: true}}, testkit.Step{Text: "success"})
	s, e := seasprak.CreateAgentSession(context.Background(), session.Options{Workspace: t.TempDir(), StateRoot: "memory", Profile: session.ProfileMemory, Model: fake, Limits: model.Limits{TraceLogicalModelCalls: 1, TraceTransportRequests: 3}})
	if e != nil {
		t.Fatal(e)
	}
...
func TestRuntimeConsumesConsecutiveSteeringAtSeparateBoundaries(t *testing.T) {
	...
	s, e := seasprak.CreateAgentSession(context.Background(), session.Options{Workspace: t.TempDir(), StateRoot: "memory", Profile: session.ProfileMemory, Model: capture, Tools: []tools.Definition{{Name: "add", Version: "v1", Schema: json.RawMessage(`{"type":"object"}`), Run: func(context.Context, json.RawMessage) (string, error) { return "ok", nil }}}})
	if e != nil {
		t.Fatal(e)
	}
	defer closeSession(t, s)
...
func TestRuntimeCloseTimeoutRetainsWriterLock(t *testing.T) {
	...
	opts := session.Options{SessionID: "locked-close", Workspace: t.TempDir(), StateRoot: t.TempDir(), Profile: session.ProfileMemory, Model: m}
	s, e := seasprak.CreateAgentSession(context.Background(), opts)
	if e != nil {
		t.Fatal(e)
	}
	r := submit(t, s)
	...
	opened, e := seasprak.OpenAgentSession(context.Background(), opts)
	if e == nil {
