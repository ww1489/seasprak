...
func TestRuntimePersistsOrderedToolResultsAndTurns(t *testing.T) {
	...
	store, _ := memory.Open("tools", history.Header{})
	manager, _ := history.NewManager(store, "tools")
	secondStarted := make(chan struct{})
	var executions atomic.Int32
	s, e := session.Start(session.Options{SessionID: "tools", Profile: session.ProfileMemory, Store: store, Model: m, Tools: []tools.Definition{{Name: "add", Version: "v1", Schema: json.RawMessage(`{"type":"object","properties":{"n":{"type":"integer"}},"required":["n"]}`), Run: func(ctx context.Context, args json.RawMessage) (string, error) {
		executions.Add(1)
		var p struct {
			N int `json:"n"`
		}
	...
	}}}, ToolInfos: []*schema.ToolInfo{seasprak.ToolInfo("add", "")}}, manager, "gen")
	if e != nil {
		t.Fatal(e)
	}
	...
	replay, e := history.NewManager(store, "tools")
	if e != nil {
		t.Fatal(e)
	}
...
func TestRuntimeUsesCompleteHistoryAndPerTraceBudget(t *testing.T) {
	...
	store, _ := memory.Open("history", history.Header{})
	manager, _ := history.NewManager(store, "history")
	s, e := session.Start(session.Options{SessionID: "history", Profile: session.ProfileMemory, Store: store, Model: m, Limits: model.Limits{TraceLogicalModelCalls: 1, TraceTransportRequests: 1}}, manager, "gen")
	if e != nil {
		t.Fatal(e)
	}
	defer closeSession(t, s)
...
func TestRuntimePublishesOriginalDurableEvents(t *testing.T) {
	...
	sub := s.SubscribeEvents(model.DefaultLimits())
	defer sub.Close()
	r := submit(t, s)
