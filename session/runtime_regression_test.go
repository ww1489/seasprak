...
func runtimeSession(t *testing.T, m einomodel.AgenticModel) (*session.AgentSession, *history.Manager) {
	t.Helper()
	store, _ := memory.Open("regression", history.Header{})
	manager, e := history.NewManager(store, "regression")
	if e != nil {
		t.Fatal(e)
	}
	s, e := session.Start(session.Options{SessionID: "regression", Profile: session.ProfileMemory, Model: m, Store: store}, manager, "gen")
	if e != nil {
		t.Fatal(e)
	}
	t.Cleanup(func() {
...
func submit(t *testing.T, s *session.AgentSession) agent.InputReceipt {
	t.Helper()
