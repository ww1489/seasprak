...
func TestSaveRejectsEscapingIDs(t *testing.T) {
	...
	manifest.ID = ".."
	if err := Save(root, "sess-ok", manifest); err == nil {
...
func TestLoadRejectsTamperedHash(t *testing.T) {
	...
	_, err = Load(root, "sess-ok", manifest.ID)
...
func TestBuildFingerprintCoversDeclaration(t *testing.T) {
	...
	if _, err := Build("", bare, ""); err == nil {
		t.Fatal("empty tool version was accepted")
	}
	builtin, err := Build("", nil, "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadRejectsUnsafeGeneration(t *testing.T) {
	_, err := Load(t.TempDir(), "sess-ok", "a:b")
	pe, ok := model.AsError(err)
	if !ok || pe.Code != model.CodeInvalidArgument {
